...
HTTP GET + requestId
  -> 从认证上下文取得 CurrentUser
  -> Application 要求 REQUESTER 或 APPROVER 业务角色
  -> Repository 以单条 SQL 联结申请、任务和决定
  -> SQL 使用 requestId + viewerId 限定申请创建者或任务受理人范围
  -> Web 仅映射申请、任务、决定、可信操作者、服务端时间和意见白名单字段
```

该流程只读取数据库中已经保存的唯一 `approval_decision`，不会根据申请或任务终态推断或伪造决定。尚无决定、申请不存在和越权访问统一映射为安全 404；普通业务 API 不提供更新、覆盖、撤销或删除最终决定的入口。查询复用 V6 的任务外键和 `UNIQUE (approval_task_id)` 约束索引，不需要新的 schema 迁移。
...
| 查询列表/详情/最终决定/审计 | 只读查询，Repository 自带用户或任务范围 | 不先加载无权数据再依赖前端过滤，不根据终态伪造最终决定 |
...
7. `DOC-004`、`US-017`、`FE-000`、`FE-010`～`FE-017`：先建立并补齐 R1 API Contract，再交付 React 前端、现有 API 集成和端到端 Demo Baseline。
