...
幂等记录的具体状态枚举和 SQL 约束在 `US-013` 实施计划中确定，但不得使用不受数据库约束保护的 `exists -> insert` 检查。
...
- 认证适配器和演示账号初始化方式（`US-003`）。
- `BusinessNumberGenerator` 的格式和唯一性策略（`US-010`）。
- `ApprovalRoutingPolicy` 的具体规则和配置来源（`US-013`）。
- 幂等执行中的“处理中”响应策略及记录状态（`US-013`、`US-015`）。
- 乐观版本与条件更新如何组合，以及数据库唯一约束的具体形式。

这些决策不能改变本文规定的可观察结果和失败不变量。

`IN_PROGRESS` 是幂等端口必须能够表达的概念结果，不预先规定 HTTP 状态码。若具体数据库事务策略使并发请求等待首个事务完成后直接 replay，也必须保持“最多一次副作用”，并在对应 Story 中记录等待、超时和处理中响应的选择。
