...
/**
 * 修改采购草稿的 HTTP 请求，使用版本和完整快照表达一次替换操作。
 *
 ...
 * @param items 完整采购项快照
 */
public record UpdateProcurementRequestRequest(
        ...
        @NotEmpty @Size(max = 100) List<@Valid ItemRequest> items)
{
    ...
    /**
     * 将已经通过协议校验的更新请求映射为应用命令。
     *
     * @return 修改采购草稿命令
     */
    public UpdateProcurementRequestCommand toCommand()
    {
        List<UpdateProcurementRequestCommand.ItemCommand> itemCommands = items.stream()
            ...
            BigDecimal estimatedUnitPrice)
    {
         ...
         * @return 更新采购项命令
         */
        private UpdateProcurementRequestCommand.ItemCommand toCommand()
        {
            return new UpdateProcurementRequestCommand.ItemCommand(
                    name,
                    categoryCode,
                    specification,
                    quantity,
